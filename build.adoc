...
choice! At least for Raspberry Pi's that support 64-bit kernels. Other
Debian based distributions for the Raspberry Pi should also work.
Raspberry Pi OS & many other Debian based distributions are available
and easy to install with the Raspberry Pi Imager Tool downloaded from
this URL, https://www.raspberrypi.com/software/ . Newer Raspberry Pi (4
& 5), with current firmware can also network boot & download/install an
OS with only a network connection on the Raspberry Pi.
...
asciidocotor is for building the documentation & pps-tools is for
PPS timing.
...
A possible workaound for the i686 (32-bit) mode build with gcc is to
