...
_gpsrinexYYYYjjjHHMMSS.obs_. You can override this filename with the
*-f* option.
...
*-g CODES*, *--gnss CODES*::
  Only output a GNSS system if the RINEX code is in CODES.
  C = BeiDou, E = Galileo G = GPS, I = IRNSS, J = QZSS, 
  R = GLONASS, S = SBAS.
*-i SECS*, *--interval SECS*::
  ...
  30.000.
*-n COUNT*, *--count COUNT*::
  Causes COUNT epochs to be output. OPUS requires a minimum of 15
