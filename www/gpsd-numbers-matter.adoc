...
The Baylonians knew about Pi 4,000 years ago.  You know Pi as an
irrational number that randomly continues forever.  At the start of the
20th century, about 500 digits of Pi were known.

Forget all that.  GNSS systems do not use that Pi.  USA Space
Systems Command has decreed, in <<IS-GPS-200>> that Pi is exactly
3.1415926535898.  About 14 decimal digits of precision.  The Galileo
<<OS_SIS_ICD>>, BeiDou <<ICD_B1I>>, and QZSS <<IS-QZSS>> also mandate
that Pi.

Of course GLONASS <ICD-GLONASS>> has a different opinion.  It uses
3.14159265358979.  One more decimal digit than GPS, et. al.
