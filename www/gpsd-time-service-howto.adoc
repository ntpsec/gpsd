...
You may add GNSS receiver protocol (e.g. "ublox=yes" or "sirf=yes").
...
The number "0.9999" is a placeholder as explained in the previous
section about configuring *ntpd*. It is *not a number to be used in
production* - it's too large. If you can't replace it with a real value,
it would be best to not use the SHM 0 source at all, keeping only the
SHM 1 source.
...
See the chrony.conf man page for more detail on the configuration options
<<CHRONY-MAN>>.
...
- [[[CHRONY-COMPARE]]] https://chrony-project.org/comparison.html[ntpd (comparison with chrony)]
