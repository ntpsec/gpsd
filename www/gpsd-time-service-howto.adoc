...
using a socket usually named /run/chrony.ttyXX.sock (where ttyXX is
replaced by the GPS receiver's device name.  Older systems may use the
/var/run directory instead of /run.  This allows multiple GPS
...
# the other sources and avoid the falseticker status
refclock SHM 0 refid GPS precision 1e-1 offset 0.9999 delay 0.2
refclock SOCK /run/chrony.ttyXX.sock refid PPS
-----------------------------------------------------------------------------

Older systems may use the /var/run directory for the scoket file instead
of /run.  /run is compliant with FHS 3.0.
