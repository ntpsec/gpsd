...
read on, after completing the above section on Initial Setup.
...
1. the receiver does not return ACK-ACK, ACK-NAK, or any UBX-LOG-INFO
message.  That means your receiver does not support logging.  Game over,
your receiver does not support logging.

2. the receiver returns somthing similar to this:
...
3. the receiver returns somthing similar to this:
...
Wait for a few more fixees to be logged, then turn off logging.
