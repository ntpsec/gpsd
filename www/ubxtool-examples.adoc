...
Sometimes the receiver will reject a setting with a simple ACK-NAK.
If you enable the Information Messages (UBX-INF-) then the
receiver may actually tell you why it rejected your setting.  This
will save you some agravation, so be sure to enable them all.

Prior to the 9-series, there appears to be no way to enable, or
disable, the UBX-INF- messages, and only a few issues will generate them.

=== With Configuration Items

If the receiver is a 9-series, or higher, then you can change receiver
settings using Configuration Items. These allow you to get, set and
delete individual settings in the receiver. Before continuing this
section, read the ubxtool man page on Configuration Items.

To enable all UBX and NMEA messages on all output ports:

----
ubxtool -P 29 -z CFG-INFMSG-UBX_I2C,15  \
...
in the limited set of ABLE options.  You can use the preset command
"-p CFG-MSG,class,ID[,rate]" to poll, or set, the message rate for any
UBX message.
...
1. u-blox 7, 8 or 9 GNSS receiver (protocol version greater than, or equal to, 15)
...
=== Checking Constellations using Configuration Items

The 9-series, and later, did not update the UBX-MON-GNSS for the newly
added constellations.  Support for UBX-CFG-GNSS was dropped entirely.
So you must use configuration items.
