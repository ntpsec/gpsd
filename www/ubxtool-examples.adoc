...
== Enabling and Disabling Informationa; Messages.

Sometimes the recevier will reject a setting with a simple ACK-NAK.
If you enable the Information Messages (CFG-INFMSG-*) then the
...
Note that you can specify the value and a decimal, hexadecimal (0x), or
binary (0b).
...
command extracts just the data that we beed.
