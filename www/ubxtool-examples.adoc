...
The u-blox GNSS receviers fall into two broad categories. The "old
...
the transistion. Gen9 still supports many of the Gen8 configuration
...
Aveiool) the examples below require that you know the protocol version
of your u-blox receiver.
...
some agravation, so be sure to enable them all.
...
If you know what port you are conencted to, you may wish to only enable
that port.
...
There will be a report for the settings in ram (the currect settings),
...
To poll the output rate of UBX-NAV-TIMELS aon the USB port we do:
...
Garden variety receviers, like the NEO-M8N do not have flsh.
...
That shows the survey is complate and the fixed point mode is active.
Oddly, you can not poll CFG-TMODE-LAT_HP for the calcualted position.
...
To check for enabled costellations and signals, get the SIGNALS group.
...
== Timepulse
Some U-blox receivers provide one or more pins delivering a Timepulse (TP)
signal. This signal can be configured in terms of pulse period (or frequency),
pulse length (or duty cycle) and polarity. It is also possible to define
different signal behavior depending on whether the receiver is locked to
 GPS time.
...
=== Timepulse period and length
Timepulse settings can be specified in two ways: period/length (in
microseconds) and frequency/duty cycle (in Hz and fractions of 2^32^-1
respectively).
...
greater than, or equal to, 23.01.  It also works differently than before.  So on
