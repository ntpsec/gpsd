...
version 3.26 or higher.  Using older gpsd versions will fail in strange
ways.
...
accuracy limit.  For quick testing w will set 30 seconds, and 10 meters,
...
The Gen9, and later, did not update the UBX-MON-GNSS for the newly added
constellations. Support for UBX-CFG-GNSS was dropped entirely. So you
must use configuration items.
...
Some receviers, like the F9T, and the F20 HPG, can be picky about what
signals can be enabled at the same time.  Illegal combinations will
cause an INFMSG to be sent, if enabled.  Sometimes it is easier to turn off
all L2 and L5, then enable what you want until your get an ACK-NAK.
