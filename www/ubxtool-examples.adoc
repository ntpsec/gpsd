...
Feel free to jump around this document to recipes that interest you,
but be sure to read all of, and comply with, the section on Initial
Setup.
...
You also need a copy of the "Interface Desscription" for your exact
u-blox device.  This document will refer to a lot of messages and
variables that are defined in that document.  Keep it open while
reading this document and refer to it often.
...
The shortcut for u-blox 8 and up:
...
the connected receiver to send the commands matched to the receiver
...
picky about competing configuration options, and fail to warn the unwary
of conflicts.  Best to always revert to factory defaults before starting
a new configuration.
...
want to enable the u-blox binary, and disable the NMEA.  Best to do it
in that order so the receiver does not go totally silent.  This is one
place where the "-P", set in UBXOPTS, is critically important, to get
the correct messages for your firmware.
...
Otherwise, unless you have a Rubidium atomic clock handy, it will be
hard for you to improve on the defaults.  If you still must fiddle, then
read on.
...
If your receiver does not return an ACK-ACK message, then it is
version 7, or earlier, and only support GPS satellites.  You can
...
receiver BeiDou, but fail to receive GLONASS.
...
Items.  These allow you to get, set or delete individual settings in
...
power.  Some GNSS receivers, like the NEO-M8B, allow the receiver to go
into sleep mode between fixes to save even more power.

Be sure you have performed all the steps in Intitial Setup before
proceeding to the logging specific steps.

=== Prerequisites

For UBX LOGGING, you need:
* u-blox 7, 8 or 9 GNSS receiver (protocol version 15+)
...
There are three possible results.  One is that the receiver does
not return ACK-ACK, ACK-NAK, or any UBX-LOG-INFO message.  That means
your receiver does not support logging.  Game over, your receiver does
not support logging.

The second is similar to this:
...
The third results is similar to this:
...
Be sure you have performed all the steps in Intitial Setup before
proceeding to the batching specific steps.

=== Prerequisites

For UBX BATCH, you need:
* u-blox 8 or 9 GNSS receiver (protocol version 23.01+)
* RAM memory
